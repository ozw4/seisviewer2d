...
function viewerState(tag = "STATE") {
  ...
  const st = {
    key1: key1Values?.[
      parseInt(document.getElementById("key1_slider")?.value || "0", 10)
    ],
    layer: document.getElementById("layerSelect")?.value,
    mode,
    stepX: latestWindowRender?.stepX,
    stepY: latestWindowRender?.stepY,
    x0,
    x1,
    renderedStart,
    renderedEnd,
...
function filenameFromContentDisposition(disposition) {
  ...
  if (utfMatch && utfMatch[1]) {
    try {
      ...
      console.warn(
        "Failed to decode UTF-8 filename from Content-Disposition:",
        err,
      );
      return utfMatch[1];
    }
...
async function exportAllPickIndexMatrixNpz() {
  ...
  const response = await fetch(
    `/export_manual_picks_all_npz?${params.toString()}`,
  );
  if (!response.ok) {
...
async function importAllPickIndexMatrixNpz() {
  if (!currentFileId) {
    alert("file_id not loaded.");
    return;
  }
  const input = document.getElementById("import-npz-all-key1");
  const modeSel = document.getElementById("import-npz-all-mode");
  const mode = modeSel && modeSel.value === "merge" ? "merge" : "replace";
  if (!input || !input.files || input.files.length === 0) {
    alert("Please choose a .npz file to import.");
    return;
  }
  ...
  const response = await fetch(
    `/import_manual_picks_all_npz?${params.toString()}`,
    {
      method: "POST",
      body: formData,
    },
  ...
  console.info("[IMPORT_NPZ]", data);
  await reloadPicksForCurrentSection();
  uiResetNonce++;
  if (typeof renderLatestView === "function") {
    renderLatestView();
  }
  alert(`Imported manual picks: inserted=${data.inserted}, mode=${data.mode}`);
}
...
function setBaseDragMode(mode) {
  ...
  if (dragBase !== next) {
    dragBase = next;
    try {
      localStorage.setItem(cfg.LS_KEYS.DRAG_BASE, next);
    } catch (_) {}
    applyDragMode();
  }
}
...
function isResetRelayout(ev) {
  ...
  if (
    !xRange ||
    !yRange ||
    !Array.isArray(sectionShape) ||
    sectionShape.length < 2
  ) {
    return false;
  }
...
function checkModeFlipAndRefetch({ immediate = false } = {}) {
  ...
  if (desired === "wiggle") {
    ...
    const needFresh =
      ...
      latestWindowRender.x0 > win.x0 ||
      latestWindowRender.x1 < win.x1 ||
      latestWindowRender.y0 > win.y0 ||
      latestWindowRender.y1 < win.y1;
...
function queueHotkeyXAxisPan(delta) {
  ...
  hotkeyXPanRaf = requestAnimationFrame(() => {
    ...
    if (!plotDiv || !curRange || typeof window.Plotly?.relayout !== "function")
      return;
    ...
    if (relayoutResult && typeof relayoutResult.catch === "function") {
      relayoutResult.catch((err) =>
        console.warn("Hotkey pan relayout failed", err),
      );
    }
  });
}
...
function runHotkeyFullView() {
  ...
  savedYRange = [
    Math.max(full.yRange[0], full.yRange[1]),
    Math.min(full.yRange[0], full.yRange[1]),
  ];
  ...
  if (plotDiv && typeof window.Plotly?.relayout === "function") {
    ...
    if (relayoutResult && typeof relayoutResult.catch === "function") {
      relayoutResult.catch((err) =>
        console.warn("Hotkey full-view relayout failed", err),
      );
    }
  }
...
function estimateAbsQuantileFromWindow(
  win,
  quantile = HOTKEY_GAIN_QUANTILE,
  maxSamples = HOTKEY_GAIN_MAX_SAMPLES,
) {
  if (!win || typeof win !== "object") return null;

  const valuesI8 = win.valuesI8 instanceof Int8Array ? win.valuesI8 : null;
  const valuesRaw =
    !valuesI8 && win.values && win.values.length != null ? win.values : null;
  const source = valuesI8 || valuesRaw;
  if (!source || source.length === 0) return null;

  const stride = Math.max(
    1,
    Math.ceil(source.length / Math.max(1, maxSamples)),
  );
  ...
  const q = Number.isFinite(qRaw)
    ? Math.max(0, Math.min(1, qRaw))
    : HOTKEY_GAIN_QUANTILE;
  const idx = Math.min(
    sampled.length - 1,
    Math.floor((sampled.length - 1) * q),
  );
  return sampled[idx];
}
...
function handleViewerHotkey(e) {
  ...
  if (keyRaw === "ArrowLeft" || keyRaw === "ArrowRight") {
    ...
    const step = Math.max(
      HOTKEY_PAN_MIN_TRACES,
      Math.round(visibleWidth * HOTKEY_PAN_RATIO),
    );
  ...
  if (key === "g") {
    ...
    if (e.shiftKey) {
      if (!setGainFromHotkey(1)) {
        console.warn(
          "[HOTKEY] Gain reset skipped: gain slider is not available",
        );
      }
    } else {
      runHotkeyAutoGain();
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) setAltPan(false);
});
window.addEventListener("pointerup", (e) => {
  if (!e.altKey) setAltPan(false);
});
  ...
  const onClick = (e) => {
    ...
    if (maybePromise && typeof maybePromise.catch === "function") {
      maybePromise.catch((err) =>
        console.warn("handlePickNormalized failed", err),
      );
    }
  };
...
function onSigmaChange() {
  localStorage.setItem(
    "sigma_ms_max",
    document.getElementById("sigma_ms_max").value,
  );
  recomputeFbPicks();
}
function onPickMethodChange() {
  localStorage.setItem(
    "pick_method",
    document.getElementById("pick_method").value,
  );
  recomputeFbPicks();
}
...
function onScalingChange() {
  ...
  const rawValue =
    sel && typeof sel.value === "string" ? sel.value.toLowerCase() : "amax";
  currentScaling = rawValue === "tracewise" ? "tracewise" : "amax";
  if (sel) sel.value = currentScaling;
  try {
    localStorage.setItem("scaling_mode", currentScaling);
  } catch (_) {}
  if (windowFetchCtrl) {
...
(function restoreWiggleUi() {
  ...
  const max = parseFloat(el.max) || 0.3;
  let v;
  if (window.cfg && typeof window.cfg.getWiggleDensity === "function") {
    ...
    v = Number.isFinite(p) ? p : parseFloat(el.value) || 0.1;
  }
...
(function hookFetchOnce() {
  ...
  window.fetch = async function (...args) {
    ...
    try {
      ...
      if (/\/picks(\?|$)/.test(url)) {
        console.debug(
          `📡 FETCH ${method} ${url} → ${res.status} (${(performance.now() - t0).toFixed(0)}ms)`,
        );
...
function zeroCrossRefine(arr, i) {
  ...
  if (!(arr[i0] <= 0 && arr[i1] > 0)) {
    if (i0 > 0 && arr[i0 - 1] <= 0 && arr[i0] > 0) {
      i0 = i0 - 1;
      i1 = i0 + 1;
    } else if (i1 < n - 1 && arr[i1] <= 0 && arr[i1 + 1] > 0) {
      i0 = i1;
      i1 = i0 + 1;
...
function currentVisibleWindow() {
  ...
  const padX =
    !forceFullExtentOnce && !!savedXRange
  ...
  const padY =
    !forceFullExtentOnce && !!savedYRange
...
async function fetchKey1Values() {
  const res = await fetch(
    `/get_key1_values?file_id=${currentFileId}&key1_byte=${currentKey1Byte}&key2_byte=${currentKey2Byte}`,
  );
  if (res.ok) {
    const data = await res.json();
    ...
    document.getElementById("key1_val_display").max =
      key1Values[key1Values.length - 1];
...
async function loadSettings() {
  ...
  currentFileId =
    params.get("file_id") || localStorage.getItem("file_id") || "";
  currentKey1Byte = parseInt(
    params.get("key1_byte") || localStorage.getItem("key1_byte") || "189",
  );
  currentKey2Byte = parseInt(
    params.get("key2_byte") || localStorage.getItem("key2_byte") || "193",
  );
  const scalingParam = (
    params.get("scaling") ||
    localStorage.getItem("scaling_mode") ||
    "amax"
  ).toLowerCase();
...
async function fetchPicks() {
  ...
  try {
    const res = await fetch(
      `/picks?file_id=${currentFileId}&key1=${encodeURIComponent(key1Val)}&key1_byte=${currentKey1Byte}&key2_byte=${currentKey2Byte}`,
    );
    if (res.ok) {
      const data = await res.json();
    ...
    D("PICKS@fetchPicks", {
      count: picks.length,
      vis: [x0, x1],
      visCount: picks.filter(
        (p) => Math.round(p.trace) >= x0 && Math.round(p.trace) <= x1,
      ).length,
...
async function fetchCurrentFileName() {
  ...
  try {
    const r = await fetch(
      `/file_info?file_id=${encodeURIComponent(currentFileId)}`,
    );
    if (!r.ok) {
      currentFileName = "";
      return;
...
async function reloadPicksForCurrentSection(key1IdxOrVal) {
  ...
  try {
    const r = await fetch(
      `/picks?file_id=${encodeURIComponent(fileId)}&key1=${encodeURIComponent(key1Val)}&key1_byte=${currentKey1Byte}&key2_byte=${currentKey2Byte}`,
    );
    if (!r.ok) return [];
    const j = await r.json();
    const arr = (j.picks || []).map((p) => ({
      trace: p.trace | 0,
      time: +p.time,
    }));
    picks = arr;
    if (typeof renderLatestView === "function") {
      try {
        ...
        console.warn("render after picks failed", e);
      }
    }
    const [x0, x1] = visibleXRng();
    D("PICKS@reload", {
...
async function fetchAndPlot() {
  ...
  predictedPicks =
    cachedEntry && cachedEntry.picks ? cachedEntry.picks.slice() : [];
  ...
  if (
    ...
    typeof window.pipelineEvents.emit === "function"
  ) {
    window.pipelineEvents.emit("section:prepare", {});
  } else if (
    window.pipelineUI &&
    typeof window.pipelineUI.prepareForNewSection === "function"
  ) {
...
function renderLatestView(startOverride = null, endOverride = null) {
  ...
  if (
    ...
    if (layer !== "raw") {
      const pipelineKeyNow = window.latestPipelineKey || null;
      if (
        (latestWindowRender.pipelineKey || null) !== (pipelineKeyNow || null)
      ) {
        return;
      }
    }
...
async function handlePickNormalized({
  trace,
  time,
  shiftKey,
  ctrlKey,
  ...
  try {
    ...
    if (ctrlKey) {
      ...
      const toDelete = picks.filter(
        (p) => Math.round(p.trace) >= start && Math.round(p.trace) <= end,
      );
      ...
      D("PICKS@handlePickNormalized:line", {
        range: [start, end],
        count: picks.length,
      });
      schedulePickOverlayUpdate();
...
async function handleRelayout(ev) {
  ...
  const shapePropKeys = Object.keys(ev).filter((k) =>
    /^shapes\[\d+\]\.(x0|x1|y0|y1|line\.color|line\.width|visible)$/.test(k),
  );
  ...
  const xa = gd?._fullLayout?.xaxis;
  const xr =
    xa && Array.isArray(xa.range)
      ? xa.range
      : [renderedStart ?? 0, renderedEnd ?? 0];
  ...
  const mergedMap = new Map(
    kept.map((p) => [
      Math.round(p.trace),
      { trace: Math.round(p.trace), time: +p.time },
    ]),
  );
  ...
  D("RELAYOUT@sync", {
    ...
    vis: [xVis0, xVis1],
    del,
    add: add.map((p) => p.trace),
    upd: upd.map((p) => p.trace),
    count: picks.length,
    sample: samplePicks(picks),
  });
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (plotDiv) {
    ...
    plotDiv.addEventListener("mouseenter", () => {
      plotHover = true;
    });
