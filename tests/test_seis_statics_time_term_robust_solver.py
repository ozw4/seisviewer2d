...
def test_robust_solver_rejects_inconsistent_trace_row_mapping() -> None:
    trace_to_row = np.full(N_TRACES, -1, dtype=np.int64)
    trace_to_row[:N_OBSERVATIONS] = np.arange(N_OBSERVATIONS, dtype=np.int64)
    trace_to_row[0] = 1

    with pytest.raises(ValueError, match='trace_to_row_index_sorted'):
        solve_time_term_robust_least_squares(
            _design(trace_to_row_index_sorted=trace_to_row),
