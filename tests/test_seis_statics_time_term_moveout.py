...
def test_compute_geometry_distance_m_returns_horizontal_distance() -> None:
...
def test_head_wave_moveout_uses_geometry_distance_over_refractor_velocity() -> None:
    ...
    np.testing.assert_array_equal(result.valid_moveout_mask_sorted, np.ones(N_TRACES))
...
def test_reciprocal_pair_mapping_finds_reverse_node_pair() -> None:
    ...
    np.testing.assert_array_equal(result.has_reciprocal_pair_mask_sorted, np.ones(N_TRACES))
...
def test_linear_offset_alias_matches_head_wave_formula() -> None:
    ...
    np.testing.assert_allclose(linear.moveout_time_s_sorted, head_wave.moveout_time_s_sorted)
