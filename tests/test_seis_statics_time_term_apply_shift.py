...
def test_delay_to_applied_shift_preserves_non_finite_delay_policy() -> None:
    delay = np.asarray([0.010, np.nan, np.inf, -np.inf], dtype=np.float64)

    np.testing.assert_allclose(
        delay_to_applied_shift_s(delay),
        [-0.010, np.nan, -np.inf, np.inf],
        equal_nan=True,
    )
