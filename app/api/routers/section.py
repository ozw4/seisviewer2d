...
def get_ntraces_for(file_id: str) -> int:
    """Return the total number of traces registered for ``file_id``."""
    ent = FILE_REGISTRY.get(file_id)
    if not ent:
        msg = f"file_id not found: {file_id}"
        raise KeyError(msg)
    reader = _resolve_reader(ent)
    ...
    if ntraces is None:
        msg = "Reader does not expose number of traces"
        raise AttributeError(msg)
    return int(ntraces)


def get_trace_seq_for(
    file_id: str, key1_idx: int, key1_byte: int
) -> NDArray[np.int64]:
    ...
    if not ent:
        msg = f"file_id not found: {file_id}"
        raise KeyError(msg)
    base_reader = _resolve_reader(ent)
    key2_byte = getattr(base_reader, "key2_byte", 193)
