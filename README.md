...
- `POST /import_manual_picks_all_npz?file_id=<FILE_ID>&key1_byte=<BYTE>&key2_byte=<BYTE>&mode=replace` (multipart `file=<NPZ>`): import manual picks from `.npz` generated by export (`picks_idx, key1_values, dt, key1_byte, key2_byte, file_id`, optional `source_sha256`)
